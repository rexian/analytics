...
public class SessionCapacityUnmarshaller {

	public static void main(String[] args) {
		List<SessionTime> sessionTimeList = new ArrayList<SessionTime>();
		JAXBContext jaxbContext;		
		OnlineJsonWriter jsonWriter = new OnlineJsonWriter();
		File outputFile =  new File("sessionCapacities.json");
		
		try {
			jaxbContext = JAXBContext.newInstance(SessionCapacities.class);
			Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
			File file =  new File("SESSION_CAPACITIES.xml");
			if(file.exists()){
				System.out.println("File exists ...." + file.getName());
			}
			SessionCapacities sessionCapacities = (SessionCapacities) jaxbUnmarshaller.unmarshal( new FileInputStream( file ));
			
			jsonWriter.writeSessionCapacityObjectToJason(sessionCapacities, outputFile);
